...
if($_COOKIE['token'] == ""){
	...
	if (mysqli_num_rows($result) > 0) {
	  ...
	  while($row = mysqli_fetch_assoc($result)) {
		...
		$sqlsu = "SELECT * FROM users WHERE username LIKE '". $row['user']."';";
		$resultus = mysqli_query($db, $sqlsu);
		$userspecific = mysqli_fetch_assoc($resultus);

		$_SESSION['user']['username'] = $row['user'];

		$_SESSION['user']['user_type'] = $userspecific['user_type'];
...
function post()
	...
	if(mysqli_query($db, $query)){
		...
		$_SESSION['error'] = "ERROR: check your database config file. mysqlerror: ". mysqli_error($db);
